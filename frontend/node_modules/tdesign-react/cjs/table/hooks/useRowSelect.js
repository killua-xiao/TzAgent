...
function useRowSelect(props, tableSelectedClasses) {
  ...
  var intersectionKeys = intersection.intersection(tSelectedRowKeys, canSelectedRows.map(function (t) {
    return get.get(t, rowKey || "id");
  }));
  ...
  function isDisabled(row, rowIndex) {
    return utils.isRowSelectedDisabled(selectColumn, row, rowIndex);
  }
  ...
  function renderSelectCell(p) {
    ...
    var checked = tSelectedRowKeys.includes(get.get(row, rowKey || "id"));
    ...
    if (column.type === "multiple") {
      var isIndeterminate = indeterminateSelectedRowKeys !== null && indeterminateSelectedRowKeys !== void 0 && indeterminateSelectedRowKeys.length ? indeterminateSelectedRowKeys.includes(get.get(row, rowKey)) : false;
      return /* @__PURE__ */React__default["default"].createElement(checkbox_index.Checkbox, _objectSpread(_objectSpread({
        indeterminate: isIndeterminate
      }, selectBoxProps), {}, {
        onClick: onCheckClick
      }));
  ...
  React.useEffect(function () {
    for (var i = 0, len = data.length; i < len; i++) {
      selectedRowDataMap.set(get.get(data[i], rowKey || "id"), data[i]);
    }
  }, [data, rowKey]);
  return {
